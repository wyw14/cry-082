...
func (m MergeIdentity) Valid() bool {
	if strings.TrimSpace(m.SiteID) == "" || strings.TrimSpace(m.PointID) == "" {
		return false
	}
	if m.Kind == KindEnvironmentalExceedance {
		return strings.TrimSpace(m.RuleID) != "" && m.RuleVersion > 0
	}
	if m.Kind == KindDeviceOffline || m.Kind == KindDeviceDrift {
		return strings.TrimSpace(m.RuleID) == ""
	}
	return false
}

func (m MergeIdentity) Key() (string, error) {
	if !m.Valid() {
