...
func (s *Service) evaluateRules(ctx context.Context, observation telemetry.Observation) error {
	...
	for _, version := range versions {
		...
		identity := alert.MergeIdentity{
			SiteID:      observation.SiteID,
			PointID:     observation.PointID,
			RuleID:      version.RuleID,
			RuleVersion: version.Version,
			Kind:        alert.KindEnvironmentalExceedance,
		}
		mergeKey, err := identity.Key()
		if err != nil {
			return err
		}
		signals = append(signals, struct {
			...
			evaluation rule.Evaluation
			mergeKey   string
		}{version: version, evaluation: evaluation, mergeKey: mergeKey})
	}
	for _, signal := range signals {
		existing, err := s.deps.Alerts.FindMergeable(ctx, signal.mergeKey, alert.KindEnvironmentalExceedance, observation.SampledAt.Add(-signal.version.MergeWindow))
		if err != nil {
			return err
		}
		if existing != nil {
		...
		created, err := alert.New(s.deps.IDs.NewID(), observation.SiteID, observation.PointID, observation.DeviceID, alert.KindEnvironmentalExceedance, signal.version.RuleID, signal.version.Version, signal.mergeKey, observation.SampledAt)
		if err != nil {
			return err
		}
