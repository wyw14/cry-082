...
func (s *Service) List(ctx context.Context, filter AlertFilter, actorID string) ([]alert.Alert, int, error) {
	...
	defer cancel()
	scope, err := alert.NewListScope(filter.SiteID, filter.Kind, filter.Status, filter.Offset, filter.Limit)
	if err != nil {
		return nil, 0, err
	}
	...
	membership, err := s.access.Membership(ctx, actorID, scope.SiteID)
	if err != nil {
		return nil, 0, err
	}
	if err := site.Require(membership, scope.SiteID, site.PermissionSiteRead); err != nil {
		return nil, 0, err
	}
	filter.SiteID = scope.SiteID
	filter.Kind = scope.Kind
	filter.Status = scope.EffectiveStatus()
	filter.Offset = scope.Offset
