...
func newTrendAccumulator(bucket time.Time, value float64) trendAccumulator {
	return trendAccumulator{bucket: bucket, minimum: value, maximum: value, total: value, samples: 1}
}

func (a trendAccumulator) add(value float64) trendAccumulator {
	if value < a.minimum {
		a.minimum = value
	}
	if value > a.maximum {
...
func (a trendAccumulator) point() TrendPoint {
	return TrendPoint{
		Bucket:  a.bucket,
		Minimum: a.minimum,
		Maximum: a.maximum,
		Average: a.total / float64(a.samples),
		Samples: a.samples,
	}
}
...
func (s *Service) Trend(ctx context.Context, siteID, pointID string, metric telemetry.Metric, start, end time.Time, bucket time.Duration) ([]TrendPoint, error) {
	...
	aggregates := map[time.Time]trendAccumulator{}
	for _, observation := range observations {
		if observation.PointID != pointID || observation.Metric != metric || observation.Quality == telemetry.QualityQuarantined {
			continue
		}
		key := observation.EffectiveTime().Truncate(bucket)
		current, exists := aggregates[key]
		if !exists {
			aggregates[key] = newTrendAccumulator(key, observation.Value)
			continue
		}
		aggregates[key] = current.add(observation.Value)
	}
