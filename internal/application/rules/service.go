...
func (s *Service) Activate(ctx context.Context, siteID, ruleID, actorID, reason, requestID string, version int64) (rule.Version, error) {
	...
	if err := latest.Activate(s.clock.Now()); err != nil {
		return rule.Version{}, err
	}
	err = transaction.Execute(ctx, s.tx, func(txctx context.Context) error {
...
func (s *Service) RequestRecalculation(ctx context.Context, siteID, ruleID, actor, reason string, fromVersion, toVersion int64, start, end time.Time) (job rule.Recalculation, err error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	if err := s.authorizeRecalculation(ctx, actor, siteID); err != nil {
		return rule.Recalculation{}, err
	}
	plan := rule.RecalculationPlan{
		SiteID:      siteID,
		RuleID:      ruleID,
		ActorID:     actor,
		Reason:      reason,
	...
	job, err = plan.Schedule(s.ids.NewID(), s.clock.Now())
	if err != nil {
		return rule.Recalculation{}, err
	}
	err = s.persistRecalculation(ctx, job)
	return job, err
}

func (s *Service) authorizeRecalculation(ctx context.Context, actor, siteID string) error {
	membership, err := s.access.Membership(ctx, actor, siteID)
	if err != nil {
		return err
	}
	return site.Require(membership, siteID, site.PermissionRuleManage)
}

func (s *Service) persistRecalculation(ctx context.Context, job rule.Recalculation) error {
	return transaction.Execute(ctx, s.tx, func(txctx context.Context) error {
		if err := s.rules.SaveRecalculation(txctx, job); err != nil {
			return err
		}
		return s.outbox.Enqueue(txctx, "rule.recalculation.requested", job.ID, map[string]any{"job_id": job.ID, "site_id": job.SiteID, "status": job.Status})
	})
}
