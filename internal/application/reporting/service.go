...
func (s *Service) Export(ctx context.Context, siteID, actor, format string, reportIDs []string) (created report.Export, err error) {
	...
	defer cancel()
	if err := s.authorizeExport(ctx, siteID, actor); err != nil {
		return report.Export{}, err
	}
	requested, storedFile, err := s.prepareExport(ctx, siteID, actor, format, reportIDs)
	if err != nil {
		return report.Export{}, err
	}
	if err := requested.MarkAnnounced(s.clock.Now()); err != nil {
		return report.Export{}, err
	}
	if err := s.notifications.Notify(ctx, actor, "监管导出已生成", map[string]string{"export_id": requested.ID, "checksum": requested.Checksum}); err != nil {
		return report.Export{}, err
	}
	if err := s.persistExport(ctx, requested, storedFile); err != nil {
		return report.Export{}, err
	}
	return requested, nil
}

func (s *Service) authorizeExport(ctx context.Context, siteID, actor string) error {
	membership, err := s.access.Membership(ctx, actor, siteID)
	if err != nil {
		return err
	}
	return site.Require(membership, siteID, site.PermissionReportExport)
}

func (s *Service) prepareExport(ctx context.Context, siteID, actor, format string, reportIDs []string) (report.Export, artifact.File, error) {
	requested, err := report.NewExport(s.ids.NewID(), siteID, format, actor, reportIDs, s.clock.Now())
	if err != nil {
		return report.Export{}, artifact.File{}, err
	}
	...
	for _, reportID := range reportIDs {
		...
		if daily.SiteID != siteID {
			return report.Export{}, artifact.File{}, site.ErrAccessDenied
		}
	...
	if err := requested.MarkReady(fileID, checksum, s.clock.Now()); err != nil {
		return report.Export{}, artifact.File{}, err
	}
	...
	return requested, storedFile, nil
}

func (s *Service) persistExport(ctx context.Context, requested report.Export, storedFile artifact.File) error {
	return transaction.Execute(ctx, s.tx, func(txctx context.Context) error {
		...
		entry, err := audit.New(s.ids.NewID(), requested.SiteID, requested.RequestedBy, "api", "regulatory-export.created", "regulatory-export", requested.ID, "generate regulatory export", "", nil, map[string]string{"format": requested.Format, "checksum": requested.Checksum}, s.clock.Now())
		if err != nil {
			return err
		}
		...
		return s.outbox.Enqueue(txctx, "regulatory-export.ready", requested.ID, map[string]any{"export_id": requested.ID, "user_id": requested.RequestedBy, "checksum": requested.Checksum})
	})
}
